...
# OLD: arg should go from 100 to 1099: 10 designs, 100 subparts per design
# CHANGED: 200 instances, each instance runs 5 repetitions.
# Groups of 0-199, 200-399, etc. for each design.
arg <- as.numeric(commandArgs(trailingOnly = TRUE))
nstream <- 200 # number of parallel random number streams
...
sim_run <- function(n, T, des_num, reps, nboot, taus = 1:3 / 4, alpha = 0.1) {
  ...
  for (j in 1:reps) {
    ...
    for (k in 1:ntaus) {
      ...
      # m_pCI[j, , k] <- c(2 * e_coef[j, k] - quantile(m_coef[j, , k], 1 - al2),
      #                     2 * e_coef[j, k] - quantile(m_coef[j, , k], al2)) #
      # this form of percentile CI works badly
  ...
  sim_ans <- list(e_coef = e_coef, e_se = e_se, e_cover = e_cover,
                  ...
                  m_cover_t = m_cover_t, taus = taus, true_beta = true_beta,
...
Tvec <- nvec / nT_prop
sims <- mapply(FUN = sim_run, nvec, Tvec, MoreArgs = list(des_num = dnum,
                reps = simreps, nboot = nboot, taus = taus, alpha = al),
                SIMPLIFY = FALSE)
